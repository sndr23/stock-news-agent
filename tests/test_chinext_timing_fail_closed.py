...
盘中行情/外盘任一缺失时，硬风控此前默认"没有风险"（fail-open）——
0.0 被当成真实平盘送入 `defensive_state`，断流当天可能照样输出高仓位且推送
看不出异常。改动后：
  1. `gather_context` 缺失显式化：盘中/外盘失败置 None（不再用 0.0 冒充）；
...
FIX-20260918-01 变更：ERP 估值极端滤波整体下线，`erp_pctile` 不再进入风险
输入清单——即使序列缺失/极贵也不再产生 `erp_missing` 或估值封顶。
"""
...
def test_score_all_erp_no_longer_triggers_missing_or_cap(monkeypatch):
    """ERP 滤波已下线：极贵/缺失的 erp_pctile 既不再产生 erp_missing，也不再封顶。"""
    _neutral_chan(monkeypatch)
    ctx = _ctx()
    baseline = cf.defensive_state(
        ctx["closes"], None,
        {"risk_off": False, "basis_min_ap": None,
         "intraday_pct": 1.0, "overseas_drop": 0.0})
    # 便宜度 0.0 = PE 处于窗口顶部（旧实现会封顶 6 成）
    res = rct.score_all(_ctx(erp_pctile=[0.0] * len(ctx["closes"])))
    assert "erp_missing" not in res["caps"]["triggers"]
    assert not any("估值" in str(t) for t in res["caps"]["triggers"])
    assert res["caps"]["cap"] == baseline["cap"]

    # 序列缺失同样不再计入风控输入
    res_none = rct.score_all(_ctx(erp_pctile=None))
    assert "erp_missing" not in res_none["caps"]["triggers"]
    assert res_none["caps"]["cap"] == baseline["cap"]


def test_score_all_all_missing_lists_every_tag(monkeypatch):
    """盘中+外盘同时缺失 → 两个标签齐全且仍只封顶一次（不叠加）。"""
    _neutral_chan(monkeypatch)
    res = rct.score_all(_ctx(intraday=None, overseas_drop=None))
    trig = res["caps"]["triggers"]
    for tag in ("intraday_missing", "overseas_missing"):
        assert tag in trig
    assert res["caps"]["cap"] == rct.MISSING_INPUT_CAP
...
def test_missing_input_end_to_end_cap_then_warning(monkeypatch):
    ...
    _neutral_chan(monkeypatch)
    ctx = _ctx(intraday=None, overseas_drop=None)
    res = rct.score_all(ctx)
    assert res["caps"]["cap"] <= rct.MISSING_INPUT_CAP
...
def test_gather_context_failed_sources_are_none_not_zero(monkeypatch):
    """盘中/外盘全部失败 → 显式 None（不再以 0.0 冒充真实值）。"""
    ...
    # ERP 滤波下线后上下文不再携带估值序列
    assert "erp_pctile" not in ctx
...
def test_render_report_warns_on_missing_risk_inputs():
    ...
    res = _safe_res(cap=rct.MISSING_INPUT_CAP,
                    triggers=["intraday_missing", "overseas_missing"])
    ...
    assert "⚠ 降级告警" in txt
    assert "盘中行情" in txt and "外盘行情" in txt
    assert "intraday_missing" in txt
