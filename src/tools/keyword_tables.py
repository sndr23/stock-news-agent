...
HIGH_SIGNAL_KEYWORDS = [
    ...
    "美联储", "鲍威尔", "欧央行", "非农",
    "汇率", "人民币贬值", "美元飙升",
    # 全球流动性/套息/利率/汇率/资金面（量化资金驱动因子，2026-08-14 审核实证：
    # 8-13 日本加息预期→日元急升→套息交易平仓→A股尾盘跳水超4300股下跌，但系统全天
    # 未推任何"日本央行/日元/套息"资讯——这些词此前不在词表，预筛分仅0.14被静默丢弃。
    # 万亿级量化资金交易的是利率/利差/汇率/波动率/资金流等因子，而非个股利好，
    # 故此处仅直通 LLM 判定，最终是否推送仍由 LLM 把关，不会自动放行）
    "日本央行", "日银", "植田", "日元", "套息", "套利交易",
    "美债收益率", "国债收益率", "收益率曲线", "利差", "期限利差",
    "逆回购", "MLF", "买断式逆回购", "SLF", "公开市场", "社融", "M2", "信贷",
    "离岸人民币", "美元指数", "人民币汇率", "中间价",
    "VIX", "波动率", "基差", "贴水", "融资余额", "两融", "融资融券",
    # 宏观数据发布（2026-08-12 审核实证：美国7月CPI公布后87条相关资讯全漏推，
    # 根因是"非农"在表而 CPI/PPI 等不在——同量级宏观数据待遇不同。
    # CPI/PPI/FOMC/GDP 为英文缩写，自动进入词边界匹配，不会误伤普通文本；
    # 通胀/物价/失业率等中文词命中后仅送 LLM 判定（是否推送仍由 LLM 把关）。）
...
# ============================================================
# 科技板块宽词表（国内"降级判定"专用，与 OVERSEAS_TECH_KEYWORDS 双口径并存）
#
# 两层口径是有意设计（2026-08-19 从 real_time_push._TECH_SECTOR_WORDS 收编统一数据源）：
# - 窄表 OVERSEAS_TECH_KEYWORDS（19词）：科技"强制推送"判定路径——误推代价高，
#   宁缺勿滥（外围消费/软件新闻不触发必推）
# - 宽表 TECH_SECTOR_WORDS_WIDE（33词）：降级判定路径——误降级代价低，
#   宁可漏降级不可误伤非科技（补全窄表缺的"液冷/散热/数据中心"等，
#   实测 sectors=['液冷'] 窄表返回 False）
# ============================================================
TECH_SECTOR_WORDS_WIDE = [
...
_SIGNAL_FP_BROAD_WORDS = {
    ...
    "机构净买入", "龙虎榜",
    # 宏观数据类（2026-08-12 新增：CPI/PPI/通胀等是"数据发布"主题而非事件本身——
    # "美国7月CPI"与"德国7月CPI"若共享指纹键会互相覆盖漏推；
    # 国家+月份+数值在标题指纹中天然区分，必须退回标题指纹）
    "CPI", "PPI", "通胀", "物价", "利率决议", "FOMC", "GDP", "失业率", "就业数据",
}
