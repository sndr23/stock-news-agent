...
当前生产组合由 ``chinext_factors.py`` 提供核心层：9 个注册因子中，
``value_erp`` 当前关闭，因此实际参与核心分数的是 8 个有效因子；本模块提供
...
def decide_position(score: float, cap: float, prev: dict,
    ...
    target = min(raw, cap)
    # 只有 cap 真正把目标仓位压低时，才标记为硬风控降档。
    # score_to_tier(score) 与 raw 的差异来自滞回带，不代表 cap 生效。
    capped = cap < raw - 1e-9
    note = []
