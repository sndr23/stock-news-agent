...
def _normalize_title(title: str) -> str:
    ...
    2026-09-07：保留数字间的小数点——"2.9个基点"曾被规范化成"29个基点"，
    同一数字的指纹分裂（2026-09-05 美债条目实测）。非数字间的点仍剥除
    （英文句尾句点等）。
    """
    t = re.sub(r"[^\u4e00-\u9fa5A-Za-z0-9.]+", "", title or "")
    t = re.sub(r"(?<!\d)\.|\.(?!\d)", "", t)  # 仅保留数字间的小数点
    return t[:40]
...
_NOISE_COLUMN_MARKERS = [
    ...
    # 分支1 的"栏目内嵌重大事件"语义放行。
    "期货热点追踪", "局势跟踪",
    # 2026-09-04 近一周审核实证（9-01 14:17）："主力资金流向前十大个股"——
    # 纯数据播报栏目，无事件语义。
    "主力资金流向", "资金流向",
]
...
_NOISE_INDEX_MOVE = [
    ...
    "创新高", "创新低",
    # 2026-09-04 晚审计补："伦敦金现短线走低"类同族短线行情措辞。
    "短线走高", "短线走低",
]
...
_NOISE_INTRADAY_MARKERS = [
    ...
    "快速跳水", "大幅跳水", "跳水", "异动",
    "涨停", "跌停",
    # 2026-09-04 晚审计实证："智能驾驶概念集体高开 大众交通2连板""AI应用概念
    # 延续强势 易点天下涨超10%"被推送——盘面措辞缺"集体高开/延续强势/连板"。
    "集体高开", "集体低开", "延续强势", "持续下挫", "持续走强", "连板",
]
# 盘面异动例外：标题含重大公司治理/监管事件词时放行给 LLM 判定
# （2026-09-04 晚审计回放实证："涨停芯片股背后藏雷！实控人、总经理双双被留置，
# 控股股东的持股100%冻结"会被"涨停"误判盘面异动——但核心是留置/冻结重大事件，
...
_NOISE_INDEX_NAMES = _INDEX_TOKENS + ["KOSPI", "恒生指数", "综合指数"] + [
    ...
    # 2026-09-04 晚审计实证："现货黄金日内跌幅扩大至2%"（21:04 新代码时段）被推送——
    # 贵金属行情基准词缺失。事件性新闻（"央行增持黄金储备"）不含涨跌措辞不误拦。
    "现货黄金", "现货白银", "黄金期货", "白银期货", "伦敦金", "COMEX",
]
...
def _is_noise_push(news: dict, judge: dict, leader_watchlist: set) -> str:
    ...
    if any(m in title for m in _NOISE_INTRADAY_MARKERS):
        # 2026-09-04 晚审计回放实证："涨停芯片股背后藏雷！实控人、总经理双双被留置"
        # 会被"涨停"误判盘面异动——但核心是留置/冻结重大事件，盘面词仅是背景。
        # 含重大公司治理/监管事件词时放行（后续仍走强档门槛 + LLM 把关）。
        if not any(k in title for k in _MAJOR_CORPORATE_EVENT_MARKERS):
...
def _load_watchlist_stocks() -> list:
    """SNA-06（2026-09-07）：加载 watchlist stocks 原始 [{code, name}]，供异动反查。"""
    try:
        wl = json.loads((PROJECT_ROOT / "watchlist.json").read_text(encoding="utf-8"))
        stocks = wl.get("stocks", []) or []
        out = []
        for s in stocks:
            if isinstance(s, dict) and str(s.get("code", "") or "").strip():
                out.append({"code": str(s["code"]).strip(),
