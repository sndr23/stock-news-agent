...
- **系统概览与工程现状**：见 [docs/项目深度分析_20260908.md](docs/项目深度分析_20260908.md)
...
- **历史口径注（2026-09-24）**：以上两条 v5.1 时代数字（+145.6%/+83.7%，及系统说明中旧配置 +95.3%）依赖 2026-09-18 FIX-20260918-01 之前的旧估值分位实现（`git 6315776^`），仅作历史参考；当前代码同口径约 **+120.5%**。
- **复现口径注（2026-09-24）**：OOS 折边界按 K 线根数下标切分，数据起点/末端平移会整体移动折边界（实测 2014-05-23 起 +123.1% vs 2014-05-28 起 +132.4%），复现任何 OOS 数字必须固定数据窗口。
...
```bash
PUSHPLUS_TOKEN=dummy python -m pytest tests/ -q -m "unit" -p no:cacheprovider   # 当前工作区 1394 passed, 15 deselected
```

测试需提供 `PUSHPLUS_TOKEN=dummy`（或 `WECOM_WEBHOOK`）：`scripts/real_time_push.py:3437` 在无推送后端时抛错，部分用例依赖该环境变量。

CI 已接入全量 unit 测试门禁（`python -m pytest tests/ -q -m unit`，step env 提供 `PUSHPLUS_TOKEN=dummy`），防止重构/依赖升级回归。
...
- **项目深度分析（当前权威）**：[docs/项目深度分析_20260908.md](docs/项目深度分析_20260908.md)（2026-09-08 全项目审查，含 9 月推送治理/LLM 降级链核验与调度层高危发现）
- **量化系统全景分析+工程审查**：[docs/项目深度分析报告_20260822.md](docs/项目深度分析报告_20260822.md)（2026-08-22/28 口径，含 ERP 滤波附注 A）
