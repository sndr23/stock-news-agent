...
| SNA-02 | P1 数据源加固：期货基差 + 资金流替代通道 | **REVIEW**（代码+测试完成：期货 新浪→中金所 / 资金流 东财→Tushare 聚合双降级链，15 专项 mock 测试 + 910 unit 全绿；Tushare 资金流通道待 token 云端生效，见 ND-002） | agent | SNA-01（代码侧已满足） | `scripts/factor_collector.py`, `tests/**`（注：期货/资金流抓取函数实际位于 factor_collector.py 而非原计划的 margin_data.py/chinext_factors.py，Allowed Paths 已按实际修正） | 中金所/Tushare 基差源可用；东财失败时自动切换；mock 测试 + unit 全绿 |
...
- 只有 `READY` 且依赖满足的任务可领取（当前无 READY 任务：SNA-02 在 REVIEW，SNA-03/SNA-05 在 BACKLOG 等待排期）。
